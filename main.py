...
# --- Logging para alertas no Telegram ---
    ...
    def check_sms():
        ...
        while time.time() - start < PRAZO_SEGUNDOS:
            ...
            if not s or s.startswith("STATUS_WAIT"):
                ...
                continue
            if s == "STATUS_CANCEL":
                # cancelado pelo provider, apenas se não recebeu nenhum SMS
                if not info["codes"]:
            ...
            if code not in info["codes"]:
                ...
                # atualiza msg existente ou envia nova
                if info.get("sms_message_id"):
                    bot.edit_message_text(text, chat_id, info["sms_message_id"], parse_mode="Markdown", reply_markup=kb)
                else:
...
def cb_comprar(c):
    ...
    price, service = prices[serv], names[serv]
    if balance < price:
        return bot.answer_callback_query(c.id,'❌ Saldo insuficiente.',True)

    bot.edit_message_text(f'⏳ Solicitando número...',c.message.chat.id,c.message.message_id)
    resp = {}
    ...
    # teclados de cancelamento
    kb_blocked = telebot.types.InlineKeyboardMarkup().add(
        telebot.types.InlineKeyboardButton('❌ Cancelar (2m)', callback_data=f'cancel_blocked_{aid}')
    )
    kb_unlocked = telebot.types.InlineKeyboardMarkup().add(
    ...
    # countdown e atualização de markup após 2m
    def countdown():
        for minute in range(PRAZO_MINUTOS):
            ...
            info = status_map.get(aid)
            if not info: return
            # sempre atualiza contagem mesmo após SMS
            new_text = (
                f"📦 {service}\n"
                f"☎️ Número: `{full}`\n"
                f"☎️ Sem DDI: `{short}`\n\n"
            ...
            # escolhe teclado desbloqueado após 2 minutos
            markup = kb_blocked if minute < 2 else kb_unlocked
            try:
                bot.edit_message_text(
                    new_text, info['chat_id'], info['message_id'],
                    parse_mode='Markdown', reply_markup=markup
                )
            except:
                pass

    # auto-cancelamento após prazo, apenas se sem SMS
    def auto_cancel():
        ...
        if info and not info.get('codes') and not info.get('canceled_by_user'):
            cancelar_numero(aid)
            alterar_saldo(
                info['user_id'], carregar_usuarios()[str(info['user_id'])]['saldo'] + info['price']
            )
            # deleta mensagem
            try:
        ...
        if payment.get('status') == 'approved':
            ...
            if ':' in ext:
                ...
                try:
                    ...
                    logger.error(f"Erro ao processar external_reference: {ext}")
    return '', 200
