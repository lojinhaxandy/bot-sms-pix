...
def spawn_sms_thread(aid):
    ...
    def check_sms():
        ...
        while time.time() - start < PRAZO_SEGUNDOS:
            ...
            # ignora STATUS_WAIT ou se usuário cancelou
            if info["canceled_by_user"] or not s or s.startswith("STATUS_WAIT"):
                time.sleep(5)
                continue
            if s == "STATUS_CANCEL":
                # cancelado pelo provedor, devolve se ainda não recebeu SMS
                if not info["codes"]:
            ...
            if code not in info["codes"]:
                ...
                # monta texto com todos os códigos recebidos
...
def cb_comprar(c):
    uid, serv = c.from_user.id, c.data.split("_")[1]
    criar_usuario(uid)
    ...
    price, service = precos[serv], names[serv]
    if balance < price:
        return bot.answer_callback_query(c.id, "❌ Saldo insuficiente.", True)

    bot.edit_message_text(
        f"⏳ Solicitando número...", c.message.chat.id, c.message.message_id
    )
    resp = {}
    for i in range(1, 14):
        resp = solicitar_numero(idsms[serv], max_price=i)
    ...
    adicionar_numero(uid, aid)
    alterar_saldo(uid, balance - price)
    ...
    msg = bot.send_message(
        c.message.chat.id, text, parse_mode="Markdown", reply_markup=kb_blocked
    )
        ...
        for _ in range(PRAZO_MINUTOS):
            ...
            try:
                bot.edit_message_text(
                    ...
                    parse_mode="Markdown", reply_markup=kb_blocked
                )
            except:
                pass
