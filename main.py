...
def solicitar_numero(servico, max_price=None):
    ...
    try:
        ...
        txt = r.text.strip()
        logger.info(f"GET_NUMBER → {txt}")
    except Exception as e:
        logger.error(f"getNumber erro: {e}"); return {'status':'error','message':str(e)}
...
def obter_status(aid):
    try:
        ...
        logger.error(f"getStatus erro: {e}"); return None
...
def spawn_sms_thread(aid):
    ...
    def check_sms():
        ...
        while time.time() - start < PRAZO_SEGUNDOS:
            ...
            kb.add(telebot.types.InlineKeyboardButton('📲 Outro SMS', callback_data=f'retry_{aid}'))
            ...
            send_info_bot(f"📩 SMS `{code}` para serviço *{service}*")
            info['processed'] = True
            return
    ...
    if data.get('type') == 'payment':
        ...
        if pay.get('status') == 'approved':
            ...
            if ref:
                ...
                bot.send_message(chat, f"✅ Recarga de R${amt:.2f} aprovada! Saldo atualizado.")
                send_info_bot(f"💰 Usuário `{uid}` recarregou R${amt:.2f}")
    return jsonify({'status':'ok'})

# === TECLADOS & HANDLERS TELEGRAM ===
...
def cb_comprar(c):
    ...
    # contagem regressiva e cancelamento automático
    def countdown():
        rem = PRAZO_MINUTOS
...
def retry_sms(c):
    ...
    try:
        ...
        bot.answer_callback_query(c.id, '❌ Falha retry.', True)
